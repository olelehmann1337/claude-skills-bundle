...
2. Invoke the `antislop-workshop` skill (or apply the rules from `~/.claude/anti-slop-writing.md` if running standalone).
...
- `newsletter-workshop-v2` — newsletters come out of the writer; editing-workshop polishes them.
- `longformtweets-workshop-v2` — tweet drafts go through editing before posting.
- `copywriting-workshop-v2` — sales copy and emails get the 9 passes after first draft.
- `landingpage-workshop-v2` — landing page sections get edited before assembly.
- Any first-draft writing skill — editing-workshop is the universal polish layer.

**Feeds out (this skill runs BEFORE):**
- `antislop-workshop-v2` — invoked as Pass 10. Final gate before delivery.
- Publication / send / post — nothing else after the anti-slop gate.

**Does NOT feed:**
- Idea generation (`interesting-workshop-v2`) — different job, runs upstream of writing.
