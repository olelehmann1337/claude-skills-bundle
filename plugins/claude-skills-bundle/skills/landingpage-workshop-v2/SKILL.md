...
Before showing the page to the user, run the draft through the `anti-slop` skill (or `antislop-workshop`).
...
- `business-interview-workshop-v2` — generates business / ICP / voice context (into CLAUDE.md or `Context/*.md`) that Step A pulls from.
- `keywordresearch-workshop-v2` — feeds search demand and audience-language data.
- `offerarchitect-workshop-v2` — produces the Offer Map that Step 3 turns into copy.

**Runs after this skill:**
- `antislop-workshop-v2` — mandatory pass, Step 8.
- `editing-workshop-v2` — optional second polish on long pages.
- `emailsequence-workshop-v2` — builds the email sequence that drives traffic to this page.
