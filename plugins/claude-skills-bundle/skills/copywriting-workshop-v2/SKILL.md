...
### Tweet / social post
See `longformtweets-workshop` or `kieran-writing`. This skill covers everything else.
...
- `business-interview-workshop-v2` — builds business + ICP + voice context (writes to CLAUDE.md or `Context/business.md`) via guided questions.
- `vibe-keyword-research` — feeds search demand data into angles.

**Runs after this skill:**
- `anti-slop` — mandatory pass, called in Step 4.
- `editing-workshop` — optional second polish pass for longer pieces.
- `landingpage-workshop` — for full page builds with design specs.
- `emailsequence-workshop` — when the CTA leads into a sequence.

**Adjacent:**
- `longformtweets-workshop` — for social-native long-form posts.
